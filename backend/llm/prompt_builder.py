...
from backend.llm.strategy_pack_context import build_strategy_pack_injection_block
from backend.telemetry import logger

# Shared between structured-outputs and schema-example prompt paths (keep in sync).
WIDA_FLEXIBLE_DOMAIN_GUIDANCE = """
...
- Hard limit: the entire objective.student_goal string MUST be 120 characters or fewer (including the parentheses domain tag and the final period); shorten wording if needed while keeping the required domain verbs and tag format
...
- **Self-check:** Before returning the JSON, re-read every `objective.student_goal`. If any day is missing at least one of the verbs `listen`, `read`, `speak`, or `write` (or their -ing forms) **or** the sentence does not end with the parentheses domain tag plus trailing period described above, or any goal exceeds 120 characters, fix it before returning the response.
...
"""
    ...
    strategy_pack_context = build_strategy_pack_injection_block(
        grade, subject, include_category_json=True, include_wida_hints=True
    )
...
7. [OK] Each objective.student_goal is at most 120 characters (including the domain tag and trailing period)
8. [OK] No co_teaching_model string breaks JSON (no unescaped double quotes inside string values)
...
10. co_teaching_model has: model_name, rationale, wida_context, phase_plan (at least 2 phases, e.g. Warmup+Practice or Input+Closure), implementation_notes. In JSON, no string under co_teaching_model (including every phase_plan field) may contain raw double-quote characters that break parsing—escape inner quotes as backslash-double-quote or rephrase (e.g. use single quotes in prose)
