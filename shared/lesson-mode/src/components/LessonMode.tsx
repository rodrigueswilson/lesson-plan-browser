...
import { buildLessonStepsFromPhasePlan } from '../utils/offlineLessonStepBuilder';
import { extractLessonPlanContext, LessonPlanContext } from '../utils/lessonPlanContext';

const safeParseLessonJson = (value: any): any | null => {
  ...
  if (typeof value === 'string') {
    try {
      ...
      console.warn('[LessonMode] Failed to parse lesson_json value:', error);
      return null;
    }
  }
...
export function LessonMode({ currentUser, scheduleEntry, planId, day, slot, weekOf, onExit }: LessonModeProps) {
  ...
  const [lessonPlanContext, setLessonPlanContext] = useState<LessonPlanContext | null>(null);
  ...
  const loadSteps = async (planId: string, day: string, slot: number) => {
    let planDetailLessonJson: any | null = null;
    let computedPlanContext: LessonPlanContext | null = null;
    try {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        if (planResponse.data) {
          ...
          setLessonPlanData(weeklyPlan);

          planDetailLessonJson = safeParseLessonJson(planResponse.data.lesson_json);
          computedPlanContext = planDetailLessonJson
            ? extractLessonPlanContext(planDetailLessonJson, day, slot)
            : null;
          setLessonPlanContext(computedPlanContext);
        } else {
          console.warn('[LessonMode] No data in plan response');
          setLessonPlanContext(null);
        }
      } catch (planErr) {
      ...
      if (response.data && response.data.length > 0) {
        ...
        try {
          ...
          const isPhasePlanMissing =
            errorMsg.includes('phase_plan') ||
            errorMsg.includes('No phase_plan');
          const isOfflineError =
            errorMsg.includes('Failed to fetch') ||
            errorMsg.includes('standalone://') ||
            errorMsg.includes('URL scheme');

          const fallbackLessonJson =
            planDetailLessonJson ?? safeParseLessonJson(lessonPlanData?.lesson_json);
          const contextFromState =
            lessonPlanData?.id === planId ? lessonPlanContext : null;
          const resolvedContext =
            computedPlanContext ??
            contextFromState ??
            (fallbackLessonJson
              ? extractLessonPlanContext(fallbackLessonJson, day, slot)
              : null);

          const derivedSteps = buildLessonStepsFromPhasePlan({
            planId,
            day,
            slot,
            lessonJson: fallbackLessonJson,
            context: resolvedContext,
          });

          if (derivedSteps && derivedSteps.length > 0) {
            console.log('[LessonMode] Derived lesson steps from local phase_plan fallback:', derivedSteps.length);
            setSteps(derivedSteps);
            const initialAdjusted: RecalculatedStep[] = derivedSteps.map(step => ({
              ...step,
              adjustedDuration: step.duration_minutes * 60,
              originalDuration: step.duration_minutes * 60,
            }));
          ...
          if (isPhasePlanMissing || isOfflineError) {
            console.log('[LessonMode] Using default lesson steps fallback:', {
              reason: isPhasePlanMissing ? 'phase_plan_missing' : 'offline_unavailable',
              message: errorMsg,
            });
  ...
  const handleTimerStart = () => {
    console.log('[LessonMode] handleTimerStart called:', {
  ...
  const handleTimerStop = () => {
    console.log('[LessonMode] handleTimerStop called:', {
      currentStepIndex: timerState.currentStepIndex,
      remainingTime: timerState.remainingTime,
      isRunning: timerState.isRunning,
      timestamp: new Date().toISOString(),
  ...
  const formattedDate = useMemo((): string | null => {
    ...
    try {
      ...
      if (weekOfToUse.startsWith('W') || weekOfToUse.startsWith('w')) {
        const weekNumber = parseInt(weekOfToUse.substring(1), 10);
        if (isNaN(weekNumber) || weekNumber < 1 || weekNumber > 53) {
          console.error('[LessonMode] Invalid week number:', { weekOfToUse, weekNumber });
          return null;
        }
        ...
        if (isNaN(mondayDate.getTime())) {
          console.error('[LessonMode] Invalid mondayDate from week number:', { weekNumber, year });
          return null;
        }
      } else {
      ...
      <TopNavigationBar
        ...
        lessonMetadata={(() => {
          const metadata = {
            ...
            scheduleStatus: timerState.scheduleStatus, // Pass schedule status for color coding
          };
