...
export function SentenceFramesDisplay({
  ...
  const allFrames = useMemo(() => {
    if (framesStep?.sentence_frames && Array.isArray(framesStep.sentence_frames) && framesStep.sentence_frames.length > 0) {
      console.log('[SentenceFramesDisplay] Returning', framesStep.sentence_frames.length, 'frames from step');
      return framesStep.sentence_frames;
    }
    if (fallbackFrames.length > 0) {
      console.log('[SentenceFramesDisplay] Using fallback frames from lesson plan context:', fallbackFrames.length);
      return fallbackFrames;
    }
    console.log('[SentenceFramesDisplay] No frames step or fallback frames available');
    return [];
  }, [framesStep, fallbackFrames]);
