...
export function VocabularyDisplay({ steps }: VocabularyDisplayProps) {
  ...
  const vocabularyItems: ExpandableItem[] = useMemo(() => {
    if (!vocabStep) {
      console.log('[VocabularyDisplay] No vocab step found');
      return [];
    }
    ...
    if (vocabStep.vocabulary_cognates && Array.isArray(vocabStep.vocabulary_cognates)) {
      ...
      return vocabStep.vocabulary_cognates.map((vocab: any, idx: number) => ({
        id: idx,
        primaryText: vocab.english || '',
        secondaryText: vocab.portuguese || '',
        badge: vocab.is_cognate ? 'Cognate' : 'Non-Cognate',
