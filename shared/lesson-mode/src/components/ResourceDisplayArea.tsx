...
export function ResourceDisplayArea({
  ...
  const derivedPlanContext = useMemo<LessonPlanContext | null>(() => {
    if (planContext) {
      return planContext;
    }
    ...
    let lessonJson: any = lessonPlanData.lesson_json;
    if (typeof lessonJson === 'string') {
      try {
        lessonJson = JSON.parse(lessonJson);
      } catch (error) {
        console.warn('[ResourceDisplayArea] Failed to parse lesson_json for fallback context:', error);
        return null;
      }
    }
    ...
    return extractLessonPlanContext(lessonJson, day, slot);
  }, [planContext, lessonPlanData, day, slot]);
  ...
  const renderResource = () => {
    switch (activeResource) {
      ...
      case 'vocabulary':
        return <VocabularyDisplay steps={steps} planContext={derivedPlanContext} />;
      case 'sentence_frames':
        return <SentenceFramesDisplay steps={steps} planContext={derivedPlanContext} />;
      case 'lesson_card':
