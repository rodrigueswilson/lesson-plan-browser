...
class TestDatabaseSchema:
    ...
    def test_lesson_json_column_exists(self, temp_db):
        ...
        with temp_db.get_connection() as session:
            ...
            assert columns["lesson_json"] in ("TEXT", "JSON")
...
class TestDatabaseStorage:
    ...
    def test_create_weekly_plan_with_lesson_json(
        ...
        temp_db.update_weekly_plan(plan_id, lesson_json=sample_lesson_json)
    ...
    def test_get_user_plans_parses_json(
        ...
        temp_db.update_weekly_plan(plan_id1, lesson_json=sample_lesson_json)
        time.sleep(1.1)
        ...
        temp_db.update_weekly_plan(plan_id2, lesson_json=sample_lesson_json)
        ...
        for plan in plans:
            if plan.id in [plan_id1, plan_id2]:
                assert isinstance(getattr(plan, "lesson_json", None), dict)
    ...
    def test_lesson_json_can_be_none(self, temp_db, sample_user_id):
        ...
        plan = temp_db.get_weekly_plan(plan_id)
        assert getattr(plan, "lesson_json", None) is None or plan.lesson_json == ""
    ...
    def test_large_lesson_json_storage(self, temp_db, sample_user_id):
        ...
        temp_db.update_weekly_plan(plan_id, lesson_json=large_json)
...
class TestEdgeCases:
    ...
    def test_invalid_json_handling(self, temp_db, sample_user_id):
        ...
        with temp_db.get_connection() as session:
            session.execute(
                text("UPDATE weekly_plans SET lesson_json = :json WHERE id = :id"),
                {"json": "{invalid json}", "id": plan_id},
            )

        plan = temp_db.get_weekly_plan(plan_id)
        lj = getattr(plan, "lesson_json", None)
        assert isinstance(lj, str) or lj is None
    ...
    def test_empty_lesson_json(self, temp_db, sample_user_id):
        ...
        temp_db.update_weekly_plan(plan_id, lesson_json={})

        plan = temp_db.get_weekly_plan(plan_id)
        lj = getattr(plan, "lesson_json", None)
        assert lj == {} or lj is None
    ...
    def test_unicode_in_lesson_json(self, temp_db, sample_user_id):
        ...
        temp_db.update_weekly_plan(plan_id, lesson_json=unicode_json)

        plan = temp_db.get_weekly_plan(plan_id)
        assert plan.lesson_json["metadata"]["teacher_name"] == "José García"
        assert "Entenderé" in plan.lesson_json["days"]["monday"]["slots"][0]["objective"]["student_goal"]
