...
4. Skill alignment: Match the lesson's target language skill (listening/speaking/reading/writing) to the strategy's primary\_skill and skill\_weights  
...
Selection limits: Maximum 2-3 strategies per lesson to avoid cognitive overload for `[GRADE_LEVEL_VARIABLE]` learners Transfer requirement: At least 1 strategy must explicitly support Portuguese-English transfer at a developmentally appropriate level Research validation: Every selection must cite JSON evidence and cross-references
...
## **Aligned Objective Generation Protocol** **Create three aligned versions of the same core learning goal:**
...
## **Step 2: Create "I Will" Version**

* ## Rewrite core goal using "I will \[action verb\] \[learning target\]"

* ## Use \``[GRADE_LEVEL_VARIABLE]`\` appropriate vocabulary

* ## Maximum 15 words for board posting

* ## Focus on the same measurable outcome as original

## **Step 3: Develop Bilingual WIDA Version**

* ## Take the same core learning goal from Steps 1-2

* ## Apply JSON templates and phrasebanks (from above protocol) to enhance with appropriate WIDA language function

* ## Include Portuguese-English sentence frames supporting the identical outcome

* ## Specify language supports that help achieve the same learning target

## **Alignment Check: All three versions must target the identical learning outcome with different language complexity and support levels**
...
* Student-Friendly "I Will" Version: \[Same core learning goal in first person, max 15 words\]  
* Bilingual Language Objective: \[Aligned objective text using the same core learning goal\] \- Generated using JSON templates and phrasebanks with Portuguese/English frames (adapted for ‘`[GRADE_LEVEL_VARIABLE]’` complexity)
...
* \[Original content if present\]  
* ELL Support: 3-5 bullets citing specific JSON strategies by ID name (converted to natural language) with implementation steps (adapted for `[GRADE_LEVEL_VARIABLE]` developmental level)  
...
* \[Original unchanged\]  
* Bilingual Check \- Quick formative assessment aligned to selected strategies and `[GRADE_LEVEL_VARIABLE]` assessment capabilities (e.g., bilingual exit tickets, cognate sorting, L1-L2 transfer check)
...
* \[ \] Each strategy includes ID name (converted to natural language, for instance, graphic organizers instead of the JSON, ‘graphic\_organizers’) and a specific rationale tied to lesson content and grade level  
* \[ \] Bilingual objectives use the exact template structure from JSON with `[GRADE_LEVEL_VARIABLE]` complexity  
...
* \[ \] "I Will" objective derives directly from original content objective  
* \[ \] Bilingual WIDA objective enhances (not changes) the same learning goal with language supports  
* \[ \] Cognitive demand level remains consistent across all three versions  
