"""
Batch ingest for Grade 2 Science corpus (Google Doc export + ingest_to_curriculum).

Requires Drive/Docs OAuth (tools/scraper/credentials/). Run from repo root:

  python tools/db/ingest_g2_science_corpus.py
  python tools/db/ingest_g2_science_corpus.py --unit-id Science_2_U1_1SNRc-NF
  python tools/db/ingest_g2_science_corpus.py --dry-run

Corpus definition: g2_science_corpus.G2_SCIENCE_UNITS. After each unit, runs
verify_curriculum_db against the newest ingest_reports/*.json (unless --no-verify).
"""
...
def main() -> int:
    from g2_science_corpus import G2_SCIENCE_UNITS
    ...
    units = list(G2_SCIENCE_UNITS)
    if args.unit_id:
        units = [u for u in units if u.unit_id == args.unit_id]
        if not units:
            print("Unknown --unit-id; not in G2_SCIENCE_UNITS.", file=sys.stderr)
            return 2
