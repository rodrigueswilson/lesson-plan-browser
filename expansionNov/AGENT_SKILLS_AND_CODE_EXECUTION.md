...
> **Reality check (Nov 2025):** No skill folders, MCP clients, sandbox executors, or structured code execution helpers exist in the repository. The FastAPI backend still calls the LLM synchronously via `backend/llm_service.py` without RestrictedPython or tool discovery. Treat this entire document as future work.
