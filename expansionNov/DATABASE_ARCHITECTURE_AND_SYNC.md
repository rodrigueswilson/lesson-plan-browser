...
> - The production code still uses the pre-expansion SQLite schema defined in `backend/database.py`. None of the new tables/columns introduced below (`lesson_json` JSONB, `sync_status`, vector clocks, `rag_retrievals`, local cache tables, etc.) exist in the repo.  
> - Supabase is not wired into the FastAPI backend; `settings.USE_SUPABASE` defaults to false, and there is no Supabase DAO implementation.  
> - No P2P sync services, device pairing, or encryption helpers are present. Sync-related classes mentioned here should be considered **planned** work.
