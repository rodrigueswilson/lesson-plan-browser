"""Diagnose why teacher names aren't showing correctly in slots."""
import sys
import json
from backend.database import get_db
from backend.utils.metadata_utils import get_teacher_name

def diagnose_teacher_issue(plan_id: str, user_id: str):
    """Diagnose teacher name issue in merged JSON."""
    print(f"Diagnosing teacher issue for plan: {plan_id}")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("SLOT DATA IN MERGED JSON:")
    print("=" * 80)
    ...
    for day_name in day_names:
        ...
        for slot in slots:
            ...
            print(f"    get_teacher_name(metadata, slot=slot): {teacher}")
            
            # Check what get_teacher_name sees
            print(f"    get_teacher_name logic check:")
            if slot:
                slot_primary = slot.get("primary_teacher_name")
                print(f"      slot.primary_teacher_name: {slot_primary} (will use if not empty)")
                slot_teacher = slot.get("teacher_name")
                print(f"      slot.teacher_name: {slot_teacher} (will use if primary_teacher_name empty)")
                slot_first = slot.get("primary_teacher_first_name", "").strip()
                slot_last = slot.get("primary_teacher_last_name", "").strip()
                constructed = f"{slot_first} {slot_last}".strip() if (slot_first or slot_last) else None
                print(f"      constructed from first+last: {constructed} (will use if above empty)")
            
            lesson_primary = metadata.get("primary_teacher_name")
            lesson_teacher = metadata.get("teacher_name")
            print(f"      metadata.primary_teacher_name: {lesson_primary} (fallback)")
            print(f"      metadata.teacher_name: {lesson_teacher} (fallback)")
...
if __name__ == "__main__":
    ...
    diagnose_teacher_issue(plan_id, user_id)
