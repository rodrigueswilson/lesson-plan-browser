# November Expansion - Master Index (Design)

**Location**: This folder was moved from `expansionNov/` to `docs/roadmap/design/`. The main roadmap index is [../README.md](../README.md).
...
> [!CAUTION]
> **Planning Advisory**
> Many features described here (Agent Skills, RAG Service, P2P Sync) involve detailed, interlocking components. **Careful, deep planning is required** before implementation to avoid technical debt. Do not rush into coding "Skills" without first establishing the underlying Service and Database foundations.
...
> **Implementation status (Dec 2025 reality check)**  
> A review of the production codebase (`backend/api.py`, `backend/database.py`, `tools/batch_processor.py`) shows that:
> - ✅ **Foundation Layer**: `OriginalLessonPlan` and `lesson_json` storage are fully implemented and verified.
> - ✅ **Multi-Slot Processing**: Existing batch processor handles multi-slot extraction correctly.
> - ⚠️ **Database Sync**: JSONB columns exist, but P2P sync and vector clocks are still planned.
> - ❌ **RAG & Services**: `FileSearchService`, `rag_retrievals` table, and Agent Skills are **not yet implemented** and require foundational work.
> - ❌ **Vocabulary**: No dedicated vocabulary tables or approval UI exist yet.
