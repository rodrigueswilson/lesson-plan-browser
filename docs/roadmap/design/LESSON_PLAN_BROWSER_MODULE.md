...
> - ✅ **Implemented**: `LessonPlanBrowser` exists in `shared/lesson-browser` and includes robust Week, Day, and Lesson views.
> - ✅ **Basic Navigation**: "Jump to Today" and automatic "Current Lesson" detection are implemented (`fetchCurrentLesson`).
> - ⚠️ **Partial**: Schedule sorting exists (`sortLessons`), but the UI for "Schedule Order" vs "Slot View" switching is not fully distinct.
> - ❌ **Missing**: The **Filtering System** (Subject, Grade, Time Range) is completely absent. No "Quick Navigation" side panel exists.
> - ❌ **Mobile**: React Native parity is planned but not implemented (currently using a shared web view architecture).
