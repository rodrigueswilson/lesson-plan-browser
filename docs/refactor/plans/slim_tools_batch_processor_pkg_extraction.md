# Refactor: tools/batch_processor_pkg/extraction.py

## Current state

- **File:** `tools/batch_processor_pkg/extraction.py` (494 non-empty lines, same metric as count_loc physical)
- **Branch:** `refactor/slim-batch-processor-pkg-extraction` (create from `master`)
...
1. Create branch from master: `git checkout master && git pull && git checkout -b refactor/slim-batch-processor-pkg-extraction`
