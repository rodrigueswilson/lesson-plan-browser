# Refactor: backend/routers/users.py

## Current state

- **File:** `backend/routers/users.py` (483 non-empty lines, same metric as count_loc physical)
- **Branch:** `refactor/slim-routers-users` (create from `master`)
...
1. Create branch from master: `git checkout master && git pull && git checkout -b refactor/slim-routers-users`
