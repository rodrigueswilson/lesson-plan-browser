# Refactor: tools/batch_processor_pkg/combine_render/multi_slot.py

## Current state

- **File:** `tools/batch_processor_pkg/combine_render/multi_slot.py` (418 non-empty lines, same metric as count_loc physical)
- **Branch:** `refactor/slim-combine-render-multi-slot` (create from `master`)

## Public API to preserve

- **Package `__all__`** (from sibling `__init__.py`): _render_single_slot, _render_multi_slot
...
1. Create branch from master: `git checkout master && git pull && git checkout -b refactor/slim-combine-render-multi-slot`
