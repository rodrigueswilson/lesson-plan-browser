# Refactor: tools/batch_processor_pkg/signatures.py

## Current state

- **File:** `tools/batch_processor_pkg/signatures.py` (553 non-empty lines)
- **Branch:** `refactor/slim-batch-processor-pkg-signatures` (create from `master`)
...
1. ~~Paragraph/date section helpers~~ **Done:** -> `tools/batch_processor_pkg/signature_paragraph_helpers.py` (`get_paragraph_font_info`, `add_date_section_to_paragraph`); used by `add_signature_image_to_table` and `add_user_name_to_table`.
2. (Optional) Find-signature-table / teacher-signature-cell helpers in a future pass.
3. ...

## Steps (one commit per extraction)

1. Create branch from master: `git checkout master && git pull && git checkout -b refactor/slim-batch-processor-pkg-signatures`
