...
| Status          | Items                                                                                                                                                                                                                                                                     |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Done**        | Batch processor package; Database alias and optional db_path (see 1.4). Session 1 (branch `fix/test-suite-collection`): test suite collection and fixes — conftest client fixture, isolated DB fixture, httpx<0.28 pin, script-style tests converted, session/engine APIs in DB tests, User/SentenceFrame/DOCXRenderer drift fixes; pytest collects 654+ tests, 0 collection errors. Test run status: after Phase 1–3, many DB/API tests pass; full suite still has some failures (e.g. analytics, image subject filtering). |
| **In progress** | —                                                                                                                                                                                                                                                                         |
| **Not started** | Priorities 1–3 (high), 5–9 (medium), 10–13 (lower). Proceed to Session 2 (split backend/api.py) from updated master per Section 0.2.                                                                                                                                                                                                     |
...
- **Test suite collection (Session 1)** — Branch `fix/test-suite-collection`: conftest `client` fixture and isolated DB fixture (`isolated_db`, `temp_db`, `test_db`, `db`); httpx<0.28 in requirements for TestClient; script-style tests converted to test functions; DB tests use Session/engine APIs (no `session.cursor()`); User/ClassSlot/WeeklyPlan attribute access in tests; SentenceFrame `language_function` and schema assertions updated; DOCXRenderer row constants (OBJECTIVE_ROW, ANTICIPATORY_SET_ROW, etc.) and None guards for pending_hyperlinks/pending_images. Pytest collects 654+ tests with 0 collection errors. Proceed to Session 2 (split backend/api.py) from updated master.
