...
- `frontend/scripts/select-env.mjs` now syncs `env/*.env` templates into `.env.local` before every dev/build so Android/desktop builds no longer rely on ad-hoc PowerShell exports.
- `frontend/scripts/run-vite.mjs` automatically chooses `vite --mode android` whenever `TAURI_ENV_PLATFORM`/`TAURI_ENV_TARGET_TRIPLE` contains `android`, keeping standalone builds consistent on CI/Gradle.
- Cross-compilation linker settings for Android live in `frontend/src-tauri/.cargo/config.toml`, so no manual `CC_aarch64_linux_android` exports are required.
