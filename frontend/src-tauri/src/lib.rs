...
fn push_tablet_db(db_path: String) -> Result<serde_json::Value, String> {
    ...
    // Copy into app dir: run separate shell commands to avoid Windows quoting issues with long strings.
    ...
    // Verify the copied file exists and has size (for debugging "error persists").
    let verify_cmd = format!("run-as {} sh -c 'ls -la databases/lesson_planner.db 2>&1'", package_name);
    let mut adb_verify = std::process::Command::new("adb");
    adb_verify
        .arg("-s")
        .arg(&device)
