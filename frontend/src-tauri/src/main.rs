...
fn show_in_folder(path: String) -> Result<(), String> {
    #[cfg(target_os = "windows")]
    {
        ...
        // Normalize: Explorer is picky about argument formatting and separators.
        let mut p = PathBuf::from(path.replace('/', "\\"));

        // If the file doesn't exist as given, try to resolve parent folder.
        if !p.exists() {
            // If user passed a directory, still try to open it.
            // Otherwise, fall back to parent directory when possible.
        ...
        if resolved.is_dir() {
            ...
            // Pass as a single arg: "/select,<path>"
...
fn detect_single_adb_device() -> Result<String, String> {
    let mut adb_devices_cmd = Command::new("adb");
...
fn install_tablet_apk_latest() -> Result<serde_json::Value, String> {
    ...
    // Try to start the main activity (best effort; don’t fail install if start fails).
...
fn find_project_root_with_script(script_name: &str) -> Result<std::path::PathBuf, String> {
    ...
    // 2) Walk up from current exe (packaged/dev)
...
#[cfg(target_os = "android")]
fn main() {
    bilingual_lesson_planner_lib::run();
}
